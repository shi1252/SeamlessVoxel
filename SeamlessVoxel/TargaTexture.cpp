...
bool TargaTexture::Initialize(ID3D11Device* device, ID3D11DeviceContext* context, LPCTSTR fileName)
{
	int width = 0;
	int height = 0;
	...
	if (FAILED(device->CreateShaderResourceView(texture, &srvd, &srv)))
		return false;
...
bool TargaTexture::LoadTarga(LPCTSTR fileName, int& height, int& width)
{
